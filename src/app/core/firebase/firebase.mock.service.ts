...
import { StudentProgressMockDB, STUDENTPROGRESS_DB } from '../store/student-progress/student-progress.mock';
import { SubsubgoalMockDB, SUBSUBGOAL_DB } from '../store/subsubgoal/subsubgoal.mock';
import { SubgoalMockDB, SUBGOAL_DB } from '../store/subgoal/subgoal.mock';
import { RoleMockDB, ROLE_DB } from '../store/role/role.mock';
import { CourseMockDB, COURSE_DB } from '../store/course/course.mock';
...
export class FirebaseMockService implements DatabaseService {
  ...
  readonly DB = {
    'studentProgresss': inject(StudentProgressMockDB),
    'subsubgoals': inject(SubsubgoalMockDB),
    'subgoals': inject(SubgoalMockDB),
    'roles': inject(RoleMockDB),
    'courses': inject(CourseMockDB),
  ...
  constructor(
    ...
    // Init Entity Models
    this.DB['studentProgresss'].init(STUDENTPROGRESS_DB);
    this.DB['subsubgoals'].init(SUBSUBGOAL_DB);
    this.DB['subgoals'].init(SUBGOAL_DB);
    this.DB['roles'].init(ROLE_DB);
    this.DB['courses'].init(COURSE_DB);
